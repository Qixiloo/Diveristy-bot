...
async def post_chat(question: str = Form(...)):
    ...
    if question.lower().startswith("/introduction"):
        ...
        response = {
            "text": "You can now chat with me freely. Type your question or use /image and /diverse-image to generate more images. If you want to end the chat, please type < /finalize > and share with us your feedback.",
            "type": "ai"
        }
        
    elif currentStep.step == "chat" and question.lower() != "/finalize" and not question.lower().startswith("/diverse-image") and not question.lower().startswith("/image"):
    ...
    elif currentStep.step == "chat" and question.lower() != "/finalize" and question.lower().startswith("/diverse-image"):
        image_desc = question.lower().replace("/diverse-image", "").strip()
        image_urls = [generate_image_from_dalle(image_desc=image_desc, type='diverse') for _ in range(3)]
        response = {
            "text": "Your request images are created.",
            "image_urls": image_urls,
            "type": "ai",
            # "description": "We produce a variety of diverse images by expanding the prompt based on diversity dimensions such as age, ethnicity, culture, gender and identity, social interaction, as well as professional and educational settings"
        }
    
    elif currentStep.step == "chat" and question.lower() != "/finalize" and question.lower().startswith("/image"):
        image_desc = question.lower().replace("/image", "").strip()
        image_urls = [generate_image_from_dalle(image_desc=image_desc, type='image') for _ in range(3)]
        response = {
            "text": "Your request images are created.",
